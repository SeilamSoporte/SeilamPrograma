...
class Muestras
{
	...
	public static function 	updateAnotaciones ($datos, $id, $ninf) 
	{
		$obj_resultados = new sQuery();
		$result= $obj_resultados->executeQuery("SELECT * FROM orden_inf_simp WHERE Id_muestra =$id AND NInf = $ninf");
	    $row = mysql_fetch_array($result);
	    if(count($row)!=1){
	     	return $obj_resultados->executeQuery("UPDATE `orden_inf_simp` SET `Anotaciones`= '$datos' WHERE Id_muestra = $id AND NInf = $ninf");
	    }
	    else{
	     	
	     	$query = "INSERT INTO `orden_inf_simp` (Id_muestra, NInf, Anotaciones) VALUES ( '$id', $ninf,'$datos' )" ;
	     	$obj_resultados->executeQuery($query); 	// ejecuta la consulta para traer al registro 
			return $obj_resultados->getAffect();  		
	    }
	} 	
	...
	function getFirmas(){
		...
		$this->FirmaAFQ = ($row['AFQ']>0) ? $this->Usuarios($row['AFQ'])[0]: Array('Nombre'=>'&nbsp;', 'Apellido'=>'&nbsp;', 'Cargo'=>'&nbsp;' , 'foto'=>'&nbsp');
		$this->FirmaRFQ = ($row['RFQ']>0) ? $this->Usuarios($row['RFQ'])[0]: Array('Nombre'=>'&nbsp;', 'Apellido'=>'&nbsp;', 'Cargo'=>'&nbsp;' , 'foto'=>'&nbsp');
		$this->FirmaAMB = ($row['AMB']>0) ? $this->Usuarios($row['AMB'])[0]: Array('Nombre'=>'&nbsp;', 'Apellido'=>'&nbsp;', 'Cargo'=>'&nbsp;' , 'foto'=>'&nbsp');
		$this->FirmaRMB = ($row['RMB']>0) ? $this->Usuarios($row['RMB'])[0]: Array('Nombre'=>'&nbsp;', 'Apellido'=>'&nbsp;', 'Cargo'=>'&nbsp;' , 'foto'=>'&nbsp');
		$this->Firma1 = ($row['FIRMA1']>0) ? $this->Usuarios($row['FIRMA1'])[0]: Array('Nombre'=>'&nbsp;', 'Apellido'=>'&nbsp;', 'Cargo'=>'&nbsp;', 'foto'=>'&nbsp');		
		$this->Firma2 = ($row['FIRMA2']>0) ? $this->Usuarios($row['FIRMA2'])[0]: Array('Nombre'=>'&nbsp;', 'Apellido'=>'&nbsp;', 'Cargo'=>'&nbsp;', 'foto'=>'&nbsp');		
		$this->Firma3 = ($row['FIRMA3']>0) ? $this->Usuarios($row['FIRMA3'])[0]: Array('Nombre'=>'&nbsp;', 'Apellido'=>'&nbsp;', 'Cargo'=>'&nbsp;', 'foto'=>'&nbsp');						
	}
	...
	{
		$obj_user = new sQuery();
		$obj_user ->executeQuery("SELECT Nombre, Apellido, Cargo, foto FROM usuarios WHERE Id=$id");
