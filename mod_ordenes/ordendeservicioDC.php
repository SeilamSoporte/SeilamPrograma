...
$Muestras = Muestras::getMuestraDC($desde,$hasta);
...
$mpdf->SetHeader('|Orden de servicio verificación de datos en campo|('.$desde.'/'.$hasta.')');
...
<html lang="es">
  <head>
    ...
    <meta name="viewport" content="width=device-width, initial-scale=1">
		<title>Ordenes de Servicios D.C</title>  
  ...
  <body>
	...
	?>
	<?php 
		$Datos 	  	= $Parametros->getDatosx($Muestra_d['Codigo'])[0]; 	
		$Analista 	= explode("|",$Datos['Nombres'])[1];
		$Analista 	= isset($Analista) ? $Analista : '';
		$Fecha    	= $Datos['Fecha_Recoleccion'];
		$Fecha    	= isset($Fecha) ? $Fecha : ''; 
		$DatosCampo = $Parametros->getDatosCampo($Muestra_d['Codigo'],$Muestra_d['CN']);
		$Datos_Campo = explode("|", $DatosCampo[0][0]); 
	 ?>
		<?php 
		...
		?>
		<tr>
			<td colspan="11" class="muestra <?php echo $Hide ?>">
		...
		<?php 
		 ...
		 ?>
			<tr>
				...
				<td class="td td-1 item text-center">	
					<?php echo isset($Datos_Campo[$d]) ? $Datos_Campo[$d] : '' ; ?>
					<?php $d++; ?>
				</td>
				<td class="td td-1">	</td>
				<td class="td td-3">	</td>
