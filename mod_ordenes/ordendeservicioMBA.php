...
$header = ' <table style="height:50px; font-size:9pt; width:100%; border-collapse: collapse" border="1">
				<tr>
					...
					<td rowspan="4" style="width:60%; text-align: center;">Orden de servicio laboratorio de Microbiología - Aguas <br>('.$desde.'/'.$hasta.')</td>
					<td style="width:20%; font-size:7pt; border:0 ; border-right:1px solid #000; border-top:1px solid #000; padding:3px 0 0 10px ">CODIGO: F-MB-44</td>
				</tr>
				<tr>
...
/*$mpdf->SetHeader('|Orden de servicio laboratorio de Microbiología - Aguas|('.$desde.'/'.$hasta.')');
*/
...
<html lang="es">
  ...
  <body>
	...
	?>
		...
		if($Hide!='hide' && $NCat==4 || $NCat==5 || $NCat==10){
		...
		<?php 
		 	...
		 	$TipoParam 	  = explode("|",$params['Tipo']);
		 		foreach ($IdParametros as $k=>$Id) {
		 			if($TipoParam[$k] ==$tipo && ($NCat==4 || $NCat==5 || $NCat==10)){
