...
//	$zonaHoraria = date_default_timezone_get();
	...
	$Campo[2] = isset($D_campo[2]) ? $D_campo[2].'°C': 'NR';

	$Datos_campo		= (buscar($Titulo, 'ALIMENTO')==true) ? "NA" : $D_campo;
	if ($Datos_campo!="NA") 
	{
		$Datos_campo = (buscar($Titulo, 'NO')==true) ? 'pH:' . $Campo[1]. ' / Temperatura:'.$Campo[2].'°C' : 'Cloro Res:'.$Campo[0] . ' / pH:' . $Campo[1]. ' / Temperatura:'.$Campo[2];
	}  
	$TextTitulo   = strtolower($Titulo); 
    $view->contentTemplate= '../templates/contenido2.php'; // seteo el template que se va a mostrar
