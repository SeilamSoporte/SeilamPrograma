$(document).ready(function () { //cuando el html fue cargado iniciar
    ...
    function guardarDatos() {
        ...
        saveD.datosencampo = '';
        saveD.caracteristicas = '';

        saveD.cambio = $('input#ultimo_codigo').val()
        saveD.usuario_cambio = $('input#usuario_cambio').val()

        color = $("#color").val();
        olor = $("#olor").val();
        aspecto = $("#aspecto").val();

        if (saveD.estado_tiempo == null) { saveD.estado_tiempo = 1 }

        $(".datoencampo").each(function (i) {
            if (i == 0) { saveD.datosencampo = $(this).val(); } else { saveD.datosencampo = saveD.datosencampo + "|" + $(this).val(); }
        })
        $(".comparador_dc").each(function (i) {
            if (i == 0) { saveD.comparadordatosencampo = $(this).val(); } else { saveD.comparadordatosencampo = saveD.comparadordatosencampo + "|" + $(this).val(); }
        })

        saveD.caracteristicas = color + "|" + olor + "|" + aspecto
