  ...
  //		return $obj_parametro->fetchAll(); 
	  ...
	  function DetallesMuestras($nro=0, $Nro=0) // declara el constructor, si trae el numero de usuario lo busca , si no, trae todos los datos
	  {
		  ...
		  if(count($row)!=1){
			  ...
			  $this->fecha_cambio = $row['fecha_cambio'];
			  $this->usuario_cambio = $row['usuario_cambio'];
		  }
		  else
		  {		
		      $this->Codigo_M	= "";
	  ...
	  function setCambio($val)
	  { $this->cambio = $val;}
	  function setFechaCambio($val)
	  { $this->fecha_cambio = $val;}
	  function setUsuarioCambio($val)
	  { $this->usuario_cambio =$val;}
